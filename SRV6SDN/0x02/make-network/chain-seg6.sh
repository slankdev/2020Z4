...
ip netns exec R1 ip rule add prio 100 from 10.2.0.14 table 400
ip netns exec R1 ip rule add prio 100 from 10.2.0.15 table 500
ip netns exec R1 ip rule add prio 100 from 10.2.0.16 table 600
...
ip netns exec R2 ip route add 10.1.0.0/24 encap seg6 mode encap segs FF:1::1,FF:2::1,F1::10 dev net2 table 400
ip netns exec R2 ip route add 10.1.0.0/24 encap seg6 mode encap segs FF:2::1,FF:3::1,F1::10 dev net2 table 500
ip netns exec R2 ip route add 10.1.0.0/24 encap seg6 mode encap segs FF:3::1,FF:1::1,F1::10 dev net2 table 600
