...
sudo ip link set R1_net3 netns R1
sudo ip link set R1_net4 netns R1

sudo ip link set R3_net0 netns R3
sudo ip link set R3_net1 netns R3

sudo ip link set R2_net0 netns R2
sudo ip link set R2_net1 netns R2
sudo ip link set R2_net2 netns R2
sudo ip link set R2_net3 netns R2
sudo ip link set R2_net4 netns R2

sudo ip link set R4_net0 netns R4
sudo ip link set R4_net1 netns R4
...
sudo ip netns exec R1 ip addr add 10.255.2.1/24 dev R1_net3
sudo ip netns exec R1 ip addr add 10.255.4.1/24 dev R1_net4
...
sudo ip netns exec R2 ip addr add 10.255.3.1/24 dev R2_net3
sudo ip netns exec R2 ip addr add 10.255.5.1/24 dev R2_net4

sudo ip netns exec R3 ip addr add 10.255.2.2/24 dev R3_net0
sudo ip netns exec R3 ip addr add 10.255.3.2/24 dev R3_net1

sudo ip netns exec R4 ip addr add 10.255.4.2/24 dev R4_net0
sudo ip netns exec R4 ip addr add 10.255.5.2/24 dev R4_net1
...
sudo ip netns exec R1 ip link set R1_net3 up
sudo ip netns exec R1 ip link set R1_net4 up
...
sudo ip netns exec R2 ip link set R2_net4 up

sudo ip netns exec R3 ip link set R3_net0 up
sudo ip netns exec R3 ip link set R3_net1 up

sudo ip netns exec R4 ip link set R4_net0 up
sudo ip netns exec R4 ip link set R4_net1 up
...
sudo ip netns exec R4 ip route add 10.1.0.0/24 via 10.255.4.1
sudo ip netns exec R4 ip route add 10.2.0.0/24 via 10.255.4.1
